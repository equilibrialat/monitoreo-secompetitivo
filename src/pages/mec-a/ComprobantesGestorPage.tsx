...
export default function ComprobantesGestorPage() {
  ...
  const [pForm, setPForm] = useState({
    ...
    tipo_gasto: "consultoría", fuente: "seco", igv_usd: "0", entregable_id: "none",
  });
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      ...
      (supabase as any).from("entregables").select("id, actividad_codigo, nombre_producto").eq("entidad_codigo", entidadCodigo),
    ]).then(([cRes, pRes, eRes]: any[]) => {
  ...
  async function handleSaveComprobante() {
    ...
    const { error } = await (supabase as any).from("comprobantes").insert({
      ...
      entregable_id: pForm.entregable_id !== "none" ? pForm.entregable_id : null,
    });
    setSaving(false);
    if (error) { toast.error("Error al guardar"); console.error(error); }
    else {
      ...
      setPForm({ contrato_id: "none", actividad_codigo: "", fecha_documento: "", clase_documento: "FAC", numero_documento: "", ruc: "", proveedor_nombre: "", concepto: "", moneda: "USD", monto_moneda_origen: "", tipo_cambio: "1", tipo_gasto: "consultoría", fuente: "seco", igv_usd: "0", entregable_id: "none" });
      loadData();
    }
  }
  ...
  function handleExportComprobantes() {
    ...
    ws["!cols"] = [{ wch: 22 }, { wch: 26 }, { wch: 14 }, { wch: 18 }, { wch: 12 }, { wch: 40 }, { wch: 18 }, { wch: 12 }];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Comprobantes");
    const fname = `comprobantes_meca_${entidadCodigo}_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return (
    <div>
      <Header title="Contratos y Comprobantes (Mec A)" subtitle={`Iniciativa: ${iniciativa?.titulo_proyecto || ""}`} />

      <Card className="mb-4 border-primary/20">
        <CardContent className="py-4">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">Resumen financiero</p>
      ...
      <Tabs defaultValue="contratos">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={handleExportComprobantes} disabled={loading || comprobantes.length === 0}>
            ...
            <Dialog open={showComprobanteDialog} onOpenChange={setShowComprobanteDialog}>
              ...
              <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <Label className="text-xs">Entregable vinculado (opcional)</Label>
                    <Select value={pForm.entregable_id} onValueChange={v => setPForm(p => ({ ...p, entregable_id: v }))}>
                      <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Sin entregable" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none" className="text-xs">Sin entregable</SelectItem>
                        {entregables.filter(e => pForm.actividad_codigo ? e.actividad_codigo === pForm.actividad_codigo : true).map(e => (
                          <SelectItem key={e.id} value={e.id} className="text-xs">{e.actividad_codigo} — {e.nombre_producto}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                    ...
                    <TableBody>
                      {contratos.map(c => (
                        <TableRow key={c.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline" className={cn("text-[10px]", c.estado === "vigente" ? "border-green-300 text-green-700" : "border-muted text-muted-foreground")}>{c.estado}</Badge>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
